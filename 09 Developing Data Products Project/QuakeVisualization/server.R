...
#
# https://www.r-bloggers.com/downloading-and-visualizing-seismic-events-from-usgs/
...
shinyServer(function(input, output) {
  ...
  observe({
        ...
        leafletProxy("mymap", data = Quake_display()) %>%
                clearShapes() %>%
                addCircles(radius = ~10^mag/2, weight = 1, color = "#777777",
  ...
  output$hist <- renderPlot({
          hist(data(), main= "Magnitude Histogram", xlab="Magnitude", xlim=c(0,8))
  })
